        ...
        <table class="table">
            <tbody class="table">
                ...
                <tr>
                    ...
                    <td>@if(collect(request()->segments())->first() == "admin" || collect(request()->segments())->first() == "instructor")<a href="/{{ collect(request()->segments())->first() }}/questionlist/{{ $Exam->id }}/Exam">No of Question's ({{ (App\Http\Controllers\QandAController::QuestionCount($Exam->id)) ? App\Http\Controllers\QandAController::QuestionCount($Exam->id) : 0 }})</a>@else <a href="javascript:void(0);">{!! (isset(App\Http\Controllers\Front\CourseController::GetQuizResult($Exam->id)->result)) ? json_decode(App\Http\Controllers\Front\CourseController::GetQuizResult($Exam->id)->result, true)["Result"]. " (".(json_decode(App\Http\Controllers\Front\CourseController::GetQuizResult($Exam->id)->result, true)["MarksObtain"]).")" : 0 !!}</a> @endif</td>
