<table class="table">
    ...
    @if(count($QandA)) @foreach ($QandA as $qa)
        <tr>
            ...
            <td>{{ $qa->qa_title }}</td>
            <td>{{ $qa->table_name  }}</td>
            <td>{!! (collect(request()->segments())->pull(1) == 'childqa') ? "<a href='/" . collect(request()->segments())->first() . "/questionlist/".App\Http\Controllers\QandAController::QuestionData($qa->qa_cid)->exam_qa_id."/Exam'>" . App\Http\Controllers\QandAController::QuestionData($qa->qa_cid)->qa_title . "</a>" : App\Http\Controllers\QandAController::ExamData($qa->exam_qa_id, $qa->table_name)->exam_title !!}</td>
            @if(collect(request()->segments())->pull(1) != 'childqa')
